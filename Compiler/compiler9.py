...
def isx(string: str) -> bool:
    ...
    if(token[0] == TokenType.INTEGER or token[0] == TokenType.FLOAT or isStatementAlphabet(token[1])):
        return 1
    return 0
...
def checkAndGetCondPartEnd(string: str) -> list:
    ...
    for i in range(len(tokens)):
        tokens[i] = tuple(list(tokens[i]) + [i])
    ...
    for i in tokens:
        if(i == end_token):
            break
        estimated_start_point += len(i[1]) + 1
        
    if(valid):  
        f = string.find(end_token[1], estimated_start_point - 1) #+ len(end_token[1])
    
    return [valid, f]

# Only this and ONLY this except for the boilerplate code is a U.D. fn that I took from the internet
...
def findElseInString(string: str) -> int:
    tokens = tokenize(string)
    ...
    for i in range(len(tokens)):
        ...
        if(tokens[i][1] == 'else' and counter == 0):
            end_token = tokens[i]
            break
    ...
    returnValue = find_nth(string, " else ", else_occurence)
...
if __name__ == "__main__":
    ...
    print_the_tokens = False
    if(logs == 1):
        print_the_tokens = True
